...
public final class MatrixReservoirItem extends BlockItem {
    public MatrixReservoirItem(Block block, Properties properties) { super(block, properties); }

    @Override protected boolean placeBlock(BlockPlaceContext context, BlockState state) {
        if (!MatrixReservoir.canPlaceColumn(context)) return false;
        var level = context.getLevel();
        BlockPos parent = context.getClickedPos();
        BlockState[] before = new BlockState[3];
        for (int part = 0; part < 3; part++) {
            BlockPos pos = parent.above(part);
            before[part] = level.getBlockState(pos);
            if (!level.isUnobstructed(state.setValue(MatrixReservoir.PART, part), pos,
        ...
        try {
            for (int part = 0; part < 3; part++) {
                BlockState placed = state.setValue(MatrixReservoir.PART, part);
            ...
            if (!complete) {
                ...
                for (int part = 0; part < 3; part++) {
                    ...
                    owned[part] = current == state.setValue(MatrixReservoir.PART, part)
                        || (part < placedParts && current.isAir());
                }
                for (int part = 0; part < 3; part++) {
                    BlockPos pos = parent.above(part);
                    if (owned[part] && level.getBlockState(pos) == state.setValue(MatrixReservoir.PART, part))
        ...
        for (int part = 0; part < 3; part++) level.updateNeighborsAt(parent.above(part), getBlock());
        return true;
    }
}
