"""Recover release Tank geometry and its original display transforms."""
import argparse
import json
...
def generate(upstream):
    ...
    legacy = json.loads(source("blockstates/radiant_tank.json"))
    ...
    files = {path: source(path) for path in (
        "models/block/radiant_tank.obj", "models/block/radiant_tank.mtl")}
    models = {
        "models/block/radiant_tank.json": {
            "loader": "${obj_loader}:obj", "model": "arcanearchives:models/block/radiant_tank.obj",
            ...
            "display": display},
        "models/item/radiant_tank.json": {"parent": "arcanearchives:block/radiant_tank"},
        "blockstates/radiant_tank.json": {"variants": {"": {"model": "arcanearchives:block/radiant_tank"}}}}
    ...
    print("Ported Tank assets from " + PIN)
