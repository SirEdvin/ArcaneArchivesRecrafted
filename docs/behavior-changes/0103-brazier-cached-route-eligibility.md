...
Per-device transient cache is now implemented in `BrazierRouteCache`, keyed by modern item identity with weak destination references and the original `<1000 ms` lifetime. Complete acceptance (including upstream-style simulation) records/refreshes an entry; a still-valid entry retains its destination on fresh fallback success. Every lookup evicts expired or currently ineligible entries before the preferred transfer path. Clock-controlled native tests verify initial recording, priority at 999 ms, refresh extending lifetime, exact 1000-ms expiry and personal-mode revocation. Four-leaf build exit 0, 86s (`build/brazier-route-cache-20260912-141827.log`); native suites/artifact verifier pass (`build/brazier-route-cache-artifacts.log`). Device ownership of the cache, actual Brazier entry points and broader cache replacement/unavailability tests remain pending. This supersedes earlier missing-cache statements, not full device/gameplay acceptance.

Preferred-destination transfer prerequisite is implemented in `BrazierRoutes.insert`: a supplied preferred instance must pass current eligibility before simulation/mutation, then fresh ranking handles remainders. Simulation excludes the already-tried instance from its fresh pass so its unreserved capacity is not counted twice; real insertion retains the ordinary fresh retry. Native tests verify valid preferred priority over a higher fresh weight, personal-mode rejection and exact partial simulated remainder. Four-leaf build exit 0, 83s (`build/brazier-preferred-transfer-20260912-140240.log`); native suites/artifact verifier pass (`build/brazier-preferred-transfer-artifacts.log`). This implements transfer-side validation/conservation, not cache storage/expiry/refresh or a registered Brazier; those remain pending. Historical source/model evidence below retains its original scope.
